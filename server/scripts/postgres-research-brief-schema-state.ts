...
const expectedColumns = new Map<string, ExpectedColumn>([
  ...
  ['status', { type: 'text', nullable: 'NO', defaultValue: null }],
  ['subjectStatus', { type: 'text', nullable: 'NO', defaultValue: null }],
  ['payloadEnc', { type: 'text', nullable: 'NO', defaultValue: null }],
  ['payloadFingerprint', { type: 'text', nullable: 'NO', defaultValue: null }],
  ['sourceSetHash', { type: 'text', nullable: 'NO', defaultValue: null }],
  ['sourceCount', { type: 'integer', nullable: 'NO', defaultValue: null }],
  ['sectionCount', { type: 'integer', nullable: 'NO', defaultValue: null }],
  ['unknownCount', { type: 'integer', nullable: 'NO', defaultValue: null }],
  ['failureCount', { type: 'integer', nullable: 'NO', defaultValue: null }],
  ['version', { type: 'integer', nullable: 'NO', defaultValue: '1' }],
  ['basedOnAt', { type: 'timestamp without time zone', nullable: 'YES', defaultValue: null }],
  ['freshUntil', { type: 'timestamp without time zone', nullable: 'YES', defaultValue: null }],
...
try {
  ...
  if (!state?.research_brief) {
    process.stdout.write(state?.tenant && state.agent_run ? 'legacy' : 'uninitialized');
  } else if (!state.tenant || !state.agent_run) {
    ...
    const [columns, indexes, foreignKeys] = await Promise.all([
      prisma.$queryRawUnsafe<ColumnRow[]>(`
        SELECT column_name, data_type, is_nullable, column_default
         ...
         WHERE table_schema = 'public' AND table_name = 'ResearchBriefSnapshot'
         ORDER BY ordinal_position
      `),
      prisma.$queryRawUnsafe<IndexRow[]>(`
        SELECT indexname FROM pg_indexes
         WHERE schemaname = 'public' AND tablename = 'ResearchBriefSnapshot'
         ORDER BY indexname
      `),
      prisma.$queryRawUnsafe<Array<{
        ...
        SELECT ccu.table_name AS foreign_table,
         ...
         WHERE tc.table_schema = 'public'
           AND tc.table_name = 'ResearchBriefSnapshot'
