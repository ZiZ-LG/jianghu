...
export const CRM_FIELD_AUTHORITY: CrmAuthorityEntry[] = CrmAuthorityMapSchema.parse([
  ...
  },
  {
    ...
    consumers: {
      reads: [
        'app/src/App.tsx', 'app/src/components/AdvisorPanel.tsx', 'app/src/components/Canvas.tsx',
        'app/src/components/DeliberationDock.tsx', 'app/src/lib/mdProfile.ts', 'app/src/lib/today.ts',
        'server/src/jobs.ts', 'server/src/state.ts', 'server/src/wecom.ts',
      ],
      writes: ['server/src/mutation/commitments.ts'],
      adapters: [
        'app/src/api.ts', 'app/src/store.ts', 'app/src/types.ts', 'app/src/wireAction.ts',
        ...
        'server/src/mutation/actionScope.ts', 'server/src/mutation/compoundCommands.ts',
    ...
    shadowComparison: 'Generic state parses only the same-row Commitment fields; invalid rows are excluded from the generic projection and never fall back to legacy fields.',
    cutoverCondition: 'CORE-106 migration parity passes and CORE-107 generic commands, CAS, audit, state projection, adapter invalidation, and rollback capability tests pass.',
    stopCondition: 'New generic writes use only the Commitment command path; legacy PlanAction writes remain an explicit same-row adapter and increment generic version/scheduleVersion.',
