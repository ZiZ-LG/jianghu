...
export const CRM_FIELD_AUTHORITY: CrmAuthorityEntry[] = CrmAuthorityMapSchema.parse([
  ...
  },
  {
    ...
    consumers: {
      reads: [
        ...
        'server/src/jobs.ts', 'server/src/mcp/syncBundle.ts', 'server/src/mcpServer.ts',
        'server/src/opp.ts', 'server/src/pde/assemble.ts', 'server/src/repair.ts',
        'server/src/state.ts', 'server/src/strategy.ts',
      ],
      writes: ['app/src/wireAction.ts', 'server/src/mutate.ts', 'server/src/voice.ts'],
      adapters: ['app/src/types.ts', 'packages/domain-contracts/src/actions.ts'],
      migrations: ['app/src/data/seed.ts', 'server/src/seed-demo.ts'],
      planned: ['CORE-103 Matter lifecycle migration and projection'],
    },
  ...
  },
  {
    ...
    consumers: {
      ...
      writes: [],
      adapters: ['app/src/lib/pde/adapter.ts'],
      migrations: [],
      planned: ['CORE-113 PdeDecisionContext stage shadow migration'],
    },
    shadowComparison: 'Compare explicit PdeDecisionContext.stageKey with the legacy input without changing PDE oracle or golden output.',
    cutoverCondition: 'PDE golden, property, and shadow parity tests pass for the explicit decision context.',
    stopCondition: 'PDE assemblers read only decisionProfileRef and PdeDecisionContext.',
    removalPhase: 'G2 CORE-113 cutover; legacy path removed in G7',
  ...
  },
  {
    ...
    currentAuthority: { kind: 'legacy_path', path: 'OppRole + OpportunityMember' },
    ...
    consumers: {
      reads: [
        ...
        'app/src/components/DetailDrawer.tsx', 'app/src/components/FocusPanel.tsx',
        'app/src/components/InboxPanel.tsx', 'app/src/components/ReadonlyProfile.tsx',
        'app/src/lib/mdProfile.ts', 'app/src/lib/pde/adapter.ts', 'app/src/store.ts',
        'server/src/jobs.ts', 'server/src/opp.ts', 'server/src/pde/assemble.ts',
        'server/src/state.ts', 'server/src/suggest.ts',
      ],
      writes: ['server/src/mutate.ts', 'server/src/personMerge.ts'],
      adapters: ['app/src/types.ts', 'packages/domain-contracts/src/actions.ts'],
      migrations: [],
      planned: ['CORE-105 MatterParticipant backfill'],
    },
    shadowComparison: 'Dry-run derives participant candidates while preserving methodology-role and legacy-visibility semantics separately.',
    cutoverCondition: 'Backfill review, tenant scope, member visibility, and cross-database migration tests pass.',
    stopCondition: 'Generic participant reads and writes use only MatterParticipant.',
  ...
  },
  {
    ...
    currentAuthority: { kind: 'legacy_path', path: 'PlanAction' },
    targetAuthority: { kind: 'core_path', path: 'PlanAction.[generic Commitment fields]' },
    consumers: {
      reads: [
        'app/src/api.ts', 'app/src/components/AdvisorPanel.tsx', 'app/src/components/Canvas.tsx',
        'app/src/components/DeliberationDock.tsx', 'app/src/lib/today.ts', 'app/src/store.ts',
        'server/src/jobs.ts', 'server/src/mutation/actionScope.ts', 'server/src/state.ts',
        'server/src/wecom.ts',
      ],
      writes: ['server/src/mutate.ts'],
      adapters: ['app/src/types.ts', 'packages/domain-contracts/src/actions.ts'],
      migrations: [],
      planned: ['CORE-106 Commitment field migration', 'CORE-108 legacy consumer cutover'],
    },
    shadowComparison: 'Project each PlanAction to Commitment V2 and compare stable ID, owner, schedule, execution, and confirmation state.',
    cutoverCondition: 'All consumers support the UTC/IANA and scheduleVersion model and migration parity passes.',
    stopCondition: 'New generic commands write the single PlanAction row through the Commitment command path.',
    removalPhase: 'Physical table may remain; legacy command contraction is G7',
    forbidden: ['Creating a second Commitment master table', 'Long-term dual writes or fallback reads'],
  },
]);
