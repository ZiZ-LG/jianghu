...
write_bootstrap_marker() {
  local marker=$1 project=$2 database=$3 backup=$4 marker_dir backup_dir backup_name tmp
  [[ "$project" =~ ^[A-Za-z0-9_.-]+$ && "$database" =~ ^[A-Za-z0-9_]+$ ]] || return 1
  ...
  if ! {
    echo 'format=jianghu-bootstrap-marker-v1'
...
verify_bootstrap_marker() {
  local marker=$1 expected_project=$2 expected_database=$3 expected_backup_dir=$4
  local line_count format project database backup_name verified_at backup_root
  VERIFIED_BOOTSTRAP_BACKUP=''
  [[ -s "$marker" ]] || { echo "bootstrap marker is missing or empty" >&2; return 1; }
  line_count=$(wc -l < "$marker" | tr -d ' ')
  [[ "$line_count" == 5 ]] || { echo "bootstrap marker has invalid metadata" >&2; return 1; }
  ...
  backup_name=$(sed -n '4s/^backup=//p' "$marker")
  verified_at=$(sed -n '5s/^verified_at=//p' "$marker")
  [[ "$format" == jianghu-bootstrap-marker-v1 ]] || return 1
