...
> **2026-09-04 历史设计提示：** [ADR-004](../ADR-004-个人商机推进工作台与研发范围收敛.md) 已批准个人商机推进工作台方向；本文的团队/企业方法论扩张不再作为建设目标。既有事实、设计过程、G64111/PDE 与安全边界保留，当前范围以 ADR-004 和 [商业清单](../商业版开发待办清单v1.md) 为准。下文状态和授权均属于所标日期的历史记录。

> **2026-09-05 个人旅程承接：** [当前产品方案](2026-09-05-jianghu-personal-customer-decision-workbench.md) 复用本文曹经理的一线销售旅程，将地图—证据—行动—复盘作为核心；不承接团队带教与预测，也不要求另找真人才能继续设计。本文下方场景与实现历史保持原样。

Generated by /office-hours on 2026-08-18<br>
Revised from full Cao-manager journey on 2026-08-19<br>
Research-informed scope refinement on 2026-08-21<br>
Branch: main<br>
...
- Governance result: ADR-002 已于 2026-08-19 接受；CORE/SAAS 任务已登记，代码仍按单任务逐项批准和验收。截至 2026-08-21，CORE-101/102 已完成，CORE-103 为唯一 READY，当前无 IN_PROGRESS。
- 2026-08-21 scope refinement: 项目所有者指示“按照建议的内容，修改后续研发方案”；本次只在 ADR-002 已批准边界内增加关系干预、可解释关系信号和受控 Agent 护栏，不启动代码、不改变 CORE-103 顺序。
...
复杂销售工作台进一步采用“关系干预队列＋受控 Agent”而不是通用聊天框：系统把无下一步、关系降温、单线联系、角色覆盖缺口、情报陈旧和假设到期组织成可解释信号，每条都能下钻到原因、来源、发生时间和建议动作。首批 Agent 仅覆盖拜访前简报、会后候选抽取和关系雷达，并受版本化 Job Card、有效数据范围、动作模式、预算和审计约束；只读结果可自动生成，正式业务变化仍只能经用户命令或 Candidate 采纳事务发生。
...
### 2026-08-21 AI-native CRM 调研带来的范围校准

本地 [PMYangkun SFA CRM 报告](../../竞品分析/PMYangkun%20CRM/crm-pmyangkun-analysis.html)与其当前官方仓库说明了方法论证据卡、对话生成变更草稿和经理风险下钻的可实现性；写操作应落为待确认表单，而不是静默写库。得到大脑中“从 Management 到 Relationship”的材料把组织记忆、关系雷达和干预队列指向同一个长期价值。官方资料中的 [Backstory](https://www.backstory.ai/solutions/account-strategy)、[Affinity](https://www.affinity.co/product/relationship-intelligence)、[Day AI](https://day.ai/product)、[Clarify](https://www.clarify.ai/agents) 与 [Gong](https://www.gong.io/platform/ai-agents-for-revenue-teams)进一步提供三类可交叉验证的产品机制：账户／购买组覆盖缺口、带来源的关系强度与暖路径、以及具备触发器、权限和运行审计的专职 Agent。

这些材料只支持范围校准，不构成江湖 PMF 或竞品效果证明。厂商客户数、ROI 和效率提升不进入本设计的成功标准。此次校准采用以下取舍：

- 立即写入设计与验收：干预原因、来源下钻、关系覆盖缺口、自然语言转确认草稿、证据化带教；
- 在 G4 增加 6 个串行工程日：受控 Agent 基座和关系信号读模型；
- 继续后置且不排期：全量邮箱／日历／微信被动采集、Clay 式批量获客外联、完整售后 CRM、企业自定义 Agent Builder。
...
| `RelationshipSignal` | 保存一次可解释、可过期的关系干预信号：kind、severity、reasonCode、sourceRefs、observedAt、freshness、ruleVersion、generatedAt、expiresAt | 派生读模型／快照，不是正式 Relation、Evidence、关键人或方法论值；只读取当前用户可见的正式 Interaction/Evidence/Relation/Commitment 与已授权元数据 |
| `OpportunityPortfolioView` | 汇总本人 4–5 个活跃销售 Matter 的注意力缺口 | 派生读模型，不新增主表；读取正式数据、方法论评估与巡检结果 |
| `AgentJobDefinition` / `AgentRun` | 固定一个 Agent 的任务、触发器、输入范围、动作模式、证据要求、预算／超时、版本与逐次运行审计 | 平台支撑对象，不属于通用 CRM 主实体或 MethodologyPack；首版只注册内置 Job，不开放任意脚本或租户自定义 Agent |
...
`RelationshipSignal` V1 不生成一个无法解释的“关系健康总分”。它分维度输出互动新鲜度、已确认互动频率与双向性、单线联系风险、关键角色覆盖、可见暖路径、证据新鲜度和下一步完整性。每一条信号必须展示“为什么、依据什么、何时发生、规则版本、建议做什么”，且能下钻到当前用户有权查看的来源；来源权限被撤销后保留最小审计指纹，不泄露正文。方法论可以提供角色和阈值，但信号、提醒和运行状态仍属于复杂销售工作台。

关系信号通过统一 `InterventionItem` 展示契约进入 Today、商机组合和团队下钻。契约至少包含 `reasonCode`、用户语言解释、`sourceRefs`、`observedAt`、`ruleVersion`、建议动作和可执行目标；G3 只提供确认临期、承诺到期和无下一步，G4 再追加关系类 provider。新增 provider 不得改变 Today 的“待确认／待跟进／已完成”首层结构，也不得自动修改正式关系、关键人、假设、阶段或预测。

受控 Agent 使用统一 Job Card，至少固定：`jobKey/version`、目的、`manual | event | schedule` 触发方式、有效 Customer/Matter 范围、允许读取的来源种类、`read_only | draft | candidate` 动作模式、证据门槛、模型／连接器引用、单次预算、超时、重试和停用开关。每次 `AgentRun` 先重新执行 tenant、RBAC/capability、effective resource scope 与 sensitive ACL，并记录 Job 版本、触发者、输入／输出引用、权限版本、模型版本、成本、耗时、结果和失败原因；审计不复制敏感正文。

首批只注册三个内置 Job：`pre_meeting_brief` 为 read_only，`post_meeting_extract` 为 candidate，`relationship_radar` 为 read_only/draft。三者均不得自动外发消息、修改阶段／Forecast、确认关键人或直接写 Customer/Matter/Person/Relation/Commitment/Interaction/Evidence。企业自定义 Agent、任意网络工具和自动发送属于未来单独决策。
...
- “今日”是一张可直接操作的扁平干预列表，不做项目管理驾驶舱。首层只展示“待确认／待跟进／已完成”三种用户语言；每项可展开“为什么现在提醒、依据和时间、建议动作”。拒绝、错过、改期和取消保留为详情中的异常状态或二级操作，不增加首层认知负担。
...
启用关系雷达后，排序还可读取 `relationship_stale`、`single_threaded`、`stakeholder_coverage_gap`、`warm_path_available` 等 RelationshipSignal，但不能只展示颜色或一个总分。用户点击风险必须看到形成信号的正式互动／关系／证据引用、数据新鲜度、规则版本和建议下一步；无法下钻或来源已失效的信号不得进入高严重级别，也不得被团队预测当作确定事实。
...
- Signals / Intervention：读取分维度 RelationshipSignal、解释原因与来源、确认／忽略／稍后处理干预项；只读信号不能直接升级为正式事实；
- Portfolio：读取本人可见的商机组合、注意力缺口与数据新鲜度；
- Agent Jobs / Runs：读取内置 Job Card、启用／停用、手工触发、读取运行状态／成本／来源与失败原因；动作模式由服务端策略固定，前端不能提权；
...
- 每次 AgentRun 都必须重新解析 tenant、capability、effective resource scope 与 sensitive ACL，不能复用已过期授权缓存；Job Card 的读范围和动作模式只能进一步收窄用户权限，不能扩大权限。
- `read_only` 只产生可再生结果，`draft` 只能产生用户草稿，`candidate` 只能写统一 Candidate helper。任何 Job 试图直写正式业务对象、自动外发消息、修改 Forecast／阶段或绕过 ReviewBatch 时失败关闭并写审计。
- Agent 单次预算、超时、最大重试和连接器 allowlist 必须服务端执行；运行审计保存引用和版本，不复制原始转写、私密备注、密钥或完整 prompt。
...
- 不把关系经营压缩成一个黑盒健康分、赢率或员工排名；V1 只展示可解释维度、来源和数据缺口。
- 不把团队视图做成员工监控或自动绩效裁决工具；没有记录必须显示为数据缺口，不能直接推断销售没有工作或能力不足。
- V1 不允许 Agent 自动外发邮件／消息、自动改阶段／Forecast、自动确认关键人或绕过 Candidate 人审；不建设任意租户自定义 Agent Builder。
- V1 不建设 Clay 式批量获客、名单抓取和外联序列，也不扩成完整售后／客户成功 CRM；这些能力只作为未来集成或单独产品决策。
...
- 关系雷达按互动新鲜度、单线联系、角色覆盖、可见暖路径、证据和下一步完整性分维度输出；不存在无依据总分，每条高严重级信号都能解释原因、时间、规则版本并下钻到有权来源。
- 拜访前简报、会后抽取和关系雷达分别以 read_only、candidate、read_only/draft Job Card 运行；停用 Job 后不再触发，权限撤销立即生效，所有正式业务自动写入与未授权外发次数均为 0。
...
- Forecast 提交时对金额、币种、预计签约日做失败关闭校验，对无下一步、关系单线、关键角色缺口和证据陈旧给出带来源的软警告；用户可在填写理由后继续，系统不得自动改 Forecast。
...
- AgentJobDefinition／AgentRun 有版本化契约、动作模式、预算和权限负向测试；candidate Job 只能调用统一 Candidate helper，read_only/draft Job 无正式写路径。
- RelationshipSignal 与 InterventionItem 只读取当前范围内正式数据／允许的元数据，来源撤销、Matter 转移或成员离队后重新计算并失败关闭；信号不成为 Relation、Evidence 或方法论值的第二权威。
...
- 新方法论版本可对既有已审核 Interaction/Evidence 生成新的 Evaluation 快照并与旧版本并排比较；历史事实、旧 Evaluation 和 active binding 均不被覆盖，切换仍需 dry-run 与用户确认。
...
- 商机健康：活跃销售 Matter 中有下一步、关键人已确认、情报在新鲜度阈值内的比例；关系信号被展开、转成 Commitment、忽略和过期的分布，以及高严重级信号无法下钻来源数（目标为 0）；
- Agent 质量：三类 Job 的触发成功率、运行耗时／成本、草稿采用率、Candidate 采纳／编辑／驳回率、停用后误触发数，以及正式数据越权写入／未授权外发数（目标均为 0）；
...
| 关系雷达退化成黑盒评分 | 销售无法信任信号，经理把不完整数据当绩效 | 不做总健康分；分维度展示 reason/source/time/ruleVersion，无法下钻的信号不得升高严重级别 |
| Agent 以自动化名义绕过人审 | 正式数据、阶段、预测或外部消息被错误修改 | 固定 Job Card 与服务端 actionMode；read_only/draft/candidate 三档失败关闭；正式写入只经用户命令或 ReviewBatch |
| Agent 运行扩大敏感数据范围或成本失控 | 越权泄露、模型费用和重试不可控 | 每次重算 scope/ACL，最小 manifest、BYO key、预算／超时／重试／allowlist 与不含正文的运行审计 |
...
- 首个 Agent 工作流依赖统一 Candidate、ReviewBatch、effective-scope resolver 与 sensitive ACL；在 CORE-206 完成 Job Card／AgentRun／预算／审计契约前，现有 AI 路径不得被包装成“自主 Agent”。
- 关系雷达依赖已审核 Interaction/Evidence、正式 Relation、Commitment、StakeholderFocus 和 IntelligenceItem；SAAS-212 先实现可解释分维度信号，再由 SAAS-209 和团队视图消费，禁止各页面复制规则。
