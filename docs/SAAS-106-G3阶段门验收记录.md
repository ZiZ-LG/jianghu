...
| 无 WorkBuddy/无 G64111 仍可完成 | commercial Free 导航精确为“今日 / 客户 / 事项 / 快速记录”；MCP/方法论/legacy full-state、mutate、demo、archive、repair 均 403，Quick Capture 只消费严格中性 CRM DTO，正式业务零旁路写入 | PASS |
...
| 商业/内部物理隔离 | 只读 CLI 使用指定 env 渲染真实 Compose，比较 project/network/volume/数据库身份/备份目录/三类密钥/宿主端口，并验证 `CUSTOMER_COMMANDS_ENABLED` 从指定 env 精确透传；缺失、非法或渲染漂移均失败关闭 | PASS |
...
2. WorkBuddy/MCP、方法论命令、legacy `/api/state`、`/api/mutate`、demo、archive、repair 和 `SET_ROLE` 均返回 `403 capability_denied`；Free 启动与快速记录只读取 `/api/crm/context` 中性 DTO，Account/Opportunity/PlanAction/AuditEvent 不产生旁路写入。
...
- 商业部署必须是 `commercial + Free + METHODOLOGY_COMMANDS_ENABLED=0`；内部部署必须明确为 `internal`；两者的 `CUSTOMER_COMMANDS_ENABLED` 必须在各自 env 中显式为 `0/1` 且与 Compose 渲染值一致。
- 两者的 project、default network、`pgdata` volume、数据库身份/密码、备份目录/主密钥、JWT/AI 密钥及宿主发布端口必须全部不同。
- CLI 只输出 project/network/volume 资源名、非敏感 Customer 命令开关状态和成功标记，不回显密钥或完整 Compose JSON。

## 7. 边界与未发生事项

- 未修改 schema、migration、G64111/PDE 算法或 oracle/golden。
- 为使 CORE-115 紧急回滚真实生效，仅在 `docker-compose.yml` 增加一行 `CUSTOMER_COMMANDS_ENABLED` 透传，并同步两个 env example；未修改 `app/package.json`、lockfile、Vite 配置、受版本控制的 `app/dist/**`、主站导航或跨站入口。
