...
按 `stephen-source-registry.md` 和 `sources.ts` 中的频率检查 10 个已登记信源：
...
每次扫描只创建“发现记录”，至少包含：`sourceId`、规范原文 URL、原文标题、发布时间、抓取时间和事件键。页面不可访问、发布时间不明或来源不在登记表时，不进入自动资格判断。
...
AI 可生成摘要初稿、翻译建议、标签建议和影响分析草稿，但编辑必须核对原文；AI 产出的风险等级、信源 ID、事实类型和发布状态一律不采信。
...
字段缺失或结构非法的内容进入 `rejected`，修复后作为新处理批次重新评估，不允许人工直接把错误状态改成公开。
