...
- 当前 `stephen.lake2ocean.top` 根页、`/healthz-stephen`、`/api/` 隔离，主站、CRM、CRM health 和 `zizai.tech` 均已完成只读在线基线检查。
...
3. first-parent diff 命中 Stephen 公开构建或发布文件；
4. 同一 SHA 的 `Stephen checks` 已成功；
5. 仓库变量 `STEPHEN_RELEASE_ENABLED` 精确等于 `1`；
6. `production-stephen` Environment 允许读取专属 SSH Secrets；
7. 重新安装、类型检查、测试并构建 Stephen；
8. 产物通过备案标识、详情旅程、候选状态泄漏、文件类型和 checksum 校验；
9. Environment 审批等待结束后，再次读取仓库开关并确认 `origin/main` 仍是该 SHA；
...
12. 检查 Stephen 核心页、全部 sitemap 详情、公安图标、`/api/` 404，以及主站、CRM 和 `zizai.tech`，并通过 `/release-id.json` 精确核对 `sourceSha` 和内容 checksum；
...
helper 在切换前后都执行 Nginx 校验。`activate <sha> <lease>` 只是进入 pending：先注册 30 分钟 expiry timer，再持久化旧 `current`/`previous`，随后切链接和 reload。外部 HTTPS 冒烟与精确 release identity 全部通过后，`finalize <sha> <lease>` 才提交事务；失败时 `rollback <sha> <lease>` 不依赖失败版本内容完整性，只校验并恢复事务中记录的精确旧链接。未收到 runner 后续消息时 timer 回滚；机器重启时 recovery service 处理残留 pending。所有 Docker/Nginx/systemd 子命令受 20 秒外层 timeout 约束，release lock 最多等待 120 秒，expiry service 遇到锁竞争或瞬时容器故障会每 15 秒重试。所有链接变更发生在同一文件系统并由 `flock` 串行化。
