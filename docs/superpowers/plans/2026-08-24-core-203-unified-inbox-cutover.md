...
> **Completion:** business commit `b5b6c30e898e06e410921538b06ee978b3efd0e7`; [GitHub Actions 32820222548](https://github.com/ZiZ-LG/jianghu/actions/runs/32820222548) verified the exact SHA with 12/12 successful jobs.
...
- [x] Characterize five-class legacy Inbox DTOs, counts and ordering before changing the reader.
- [x] Add RED tests for tenant/Customer/Matter/Person/Commitment parent closure, creator quarantine, required provenance/confidence, semantic dedupe and Candidate version conflicts.
- [x] Add RED tests proving field/reminder/pending-Evidence producers cannot change formal CRM data before review.
- [x] Add RED tests proving a later conflict rolls back every earlier item in a mixed batch and returns an explicit error.
- [x] Add a production-source inventory test that permits old-table mutations only in the two Candidate helper modules, with an explicit exception for approved formal Evidence creation/deletion.
...
- [x] Correct the projection validator for all supported ChangeProposal targets (`person`, `personLog`, `oppRole`, `opportunity`, `bi`, `ucv`) and all Reminder targets (Matter, Person or Commitment by kind).
- [x] Implement `--apply` as one idempotent transaction: validate every tenant-scoped source row, create only missing linked Candidate rows, verify existing rows semantically, normalize legacy created-at ordering, verify counts/checksum and write the migration marker last.
- [x] Make `--verify` require the marker and validate bidirectional source/Candidate parity without exposing evidence, rawContent or payload text. Keep `--dry-run` read-only for pre-expansion/recovery checks.
- [x] Run the same data step on SQLite upgrade and PostgreSQL `migrate deploy` wrappers. A missing marker is recoverable by rerunning apply; invalid, partial, cross-tenant or conflicting data fails closed.
- [x] Failure-inject preflight, apply interruption, rerun, marker drift, semantic conflict and authenticated backup/restore paths on both databases.
...
- Modify: `server/tests/commitment-reminders.test.ts`

- [x] Implement tenant-scoped create/update/claim/finalize/reject helpers with Candidate CAS and same-transaction compatibility projection writes.
- [x] Keep ChangeProposal field-value conflict checks and formal Action application in the existing review transaction; release semantic pending dedupe only on terminal review.
- [x] Keep patrol deterministic and read-only with respect to formal CRM state. It may only upsert/resolve reminder Candidates plus their compatibility projections.
- [x] Route every machine `pending_review` Evidence producer through the helper. Trusted human Evidence continues to create an approved formal EvidenceEvent directly; pending Evidence deletion fails closed.
- [x] Redirect person-merge references and resolve semantic-key collisions inside the helper so Candidate and projections cannot diverge.
...
- [x] Query pending Candidate rows once, enforce EffectiveResourceScope and parent closure, and map the existing five response arrays using Candidate payload plus formal Account/Matter/Person/Signal labels. Do not read old candidate tables or add fallback.
- [x] Preserve legacy endpoint IDs and DTO shapes. Sort Person/Relation by confidence descending and Change/Reminder/Evidence by legacy createdAt descending.
- [x] Claim Candidate before formal Person/Edge/field/Evidence writes; finalize Candidate and its compatibility projection in the same transaction.
- [x] On Evidence approval, apply optional overrides, approve the compatibility EvidenceEvent, write the PDE snapshot and finalize Candidate atomically. Rejection never affects PDE/formal state.
- [x] Preserve viewer rejection and current member/owner/admin scope. Creator-share ACL remains deferred to CORE-204.
- [x] Preserve command idempotency and all-or-nothing mixed-batch semantics; stale/partial conflicts must be explicit and leave no earlier item committed.
...
- [x] Run focused Candidate/Inbox/producer/batch/merge/tenant tests, then full Server typecheck/test and PostgreSQL operations integration.
- [x] Refresh package copies with `npm ci --install-links` before unchanged Domain/G64111/PDE/App gates if a package changes; no package change was required.
- [x] Run Domain contracts, G64111, PDE kernel and App typecheck/tests. Do not run a local App production build because it writes shared `app/dist/**`; exact-SHA CI retains that isolated build gate.
- [x] Run `git diff --check`, old-table no-bypass inventory, protected-path diff check and secret-pattern scan. Assert no schema/Action/App/shared/self-cultivation/production file changed.
- [x] Commit business code independently, push and require every exact-head CI job green.
- [x] In a separate governance commit, document migration/apply/verify/rollback commands, mark CORE-203 DONE and only CORE-204 READY, push and require exact-head CI green before CORE-204 starts.
...
DATABASE_URL=file:./test.db npx vitest run \
  ...
  tests/commitment-reminders.test.ts
npm test
